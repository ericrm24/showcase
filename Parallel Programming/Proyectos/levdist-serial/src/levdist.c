...
void levdist_process_queue (distance_t distances[], queue_t* queue, bool unicode);
...
int levdist_process_dirs(levdist_t* this, int argc, char* argv[])
{
    ...
    // Process files queue
    levdist_process_queue(this->distances, this->files, this->arguments.unicode);
    ...
    if ( ! this->arguments.silent ) {
        ...
        if ( ! this->arguments.quiet ) {
            // Report elapsed time
            printf("Elapsed %.9lfs with serial version\n", walltime_elapsed(&start));
        }
    }
    queue_destroy(this->files, true);
    free(this->distances);
...
void levdist_process_queue(distance_t* distances, queue_t *queue, bool unicode)
{
    ...
    for (queue_iterator_t i = queue_begin(queue), end = queue_end(queue); i != end; i = queue_next(i)) {
        for (queue_iterator_t j = queue_next(i); j != end; j = queue_next(j)) {
            ...
            /* Compare */
            distances[distances_index].distance = levenshtein_distance(distances[distances_index].file1, distances[distances_index].file2, unicode);
