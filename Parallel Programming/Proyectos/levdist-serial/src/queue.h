...
/**
    ...
    @remarks The queue must not be empty.
*/
...
/**
 * @brief Makes sure there are at least two different files to compare.
 * @param queue Pointer to the queue to be analyzed.
 * @return Value 1 if there is only one file, value 0 if there are more.
 */
int queue_different_files(queue_t* queue);

/**
 * @brief Sorts the queue alphabetically by the path contained in it.
 * @param queue Pointer to the queue to be sorted.
 * @return Pointer to a sorted queue.
 */
