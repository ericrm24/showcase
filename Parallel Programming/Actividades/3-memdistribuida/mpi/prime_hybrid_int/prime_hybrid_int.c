...
int main(int argc, char* argv[])
{
	...
	if (my_rank == 0)
	{
		...
		for (source = 1; source < process_count; ++source)
		{
			...
			MPI_Recv(&partial_threads, 1, MPI_UNSIGNED_LONG_LONG, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			prime_count += partial_prime;
			threads += partial_threads;
		}

		printf("%zu primes found in range [%zu, %zu[ in %lf with %d processes and  %zu threads\n", prime_count, start, end, max_duration, process_count, threads);
	}
	else
	{
		MPI_Send(&prime_count, 1, MPI_UNSIGNED_LONG_LONG, 0, 0, MPI_COMM_WORLD);
		MPI_Send(&threads, 1, MPI_UNSIGNED_LONG_LONG, 0, 0, MPI_COMM_WORLD);
	}
