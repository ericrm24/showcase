...
int main(int argc, char* argv[])
{
	...
	if (my_rank == 0){
		if (argc == 3){
			if ( sscanf (argv[2], "%d", &first) != 1 || first < 0 || first > process_count){
	...
	if (my_rank == first)
	{
		if (argc >=2)
		{
			if ( sscanf (argv[1], "%d", &potato_init) != 1 || potato_init < 1 ) {
		...
		potato = potato_init;
		potato = (potato % 2 == 0) ? potato / 2 : 3*potato + 1;
		if (potato == 1){ /* Perdió inmediatamente */
			playing = false;
			++finished;
			potato = potato_init;
		}
	...
	MPI_Bcast(&potato_init, 1, MPI_INT, first, MPI_COMM_WORLD);
